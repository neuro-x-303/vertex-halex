...
// Helper for dynamic linear RGB hue cycling
...
const WebThreads = ({
  ...
  const dynamicColorRef = useRef({ enabled: dynamicColorSpeed > 0, speed: dynamicColorSpeed });

  useEffect(() => {
    dynamicColorRef.current = { enabled: dynamicColor2, speed: dynamicColorSpeed };
  }, [dynamicColor2, dynamicColorSpeed]);
  ...
  useEffect(() => {
    ...
    const loop = t => {
      ...
      // Dynamic linear RGB hue cycling for Color 2
      if (dynamicColorRef.current.enabled) {
        // Cycle hue continuously from pink/rose through cyan, violet, and emerald
        const hue = (0.88 + elapsedSeconds * dynamicColorRef.current.speed) % 1.0;
        const [r, g, b] = hslToRgb(hue, 1.0, 0.52);
